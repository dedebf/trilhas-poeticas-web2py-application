...
def list_tracks():
    ...
    grid.view_link = lambda row: A(IMG(IMG(_src=URL('static','images/search-icon.png'),_alt='test', _width='12px', _height='12px'),
...
@auth.requires_login()
def show_track():
    track_id = request.args(0) or redirect(URL('sistema','index'))
    ...
    buttons=[]
    #b_editar_trilha = INPUT(_value="Editar Trilha", _type="button", _class="btn-primary", _onclick="javascript:window.location.href = '/sistema/edit_track/'"+track_id)
...
@auth.requires_login()
def remove_point() :
    track_id = request.args(1) or redirect(URL('sistema','index'))
    if not is_my_track(track_id):
        session.flash="Esta Trilha parece não pertencer a você.";
        redirect(URL('sistema','index'))
...
def get_full_set_after_date():
    ...
    tracks = db((db.t_tracks.modified_on >= convertedDatetime)|(db.t_track_ratings.modified_on>=convertedDatetime),(db.t_track_tags.modified_on>=convertedDatetime)).select(
        ...
        join=[db.auth_user.on(db.t_tracks.f_owner==db.auth_user.id), db.t_track_ratings.on(db.t_track_ratings.f_track==db.t_tracks.id), db.t_track_tags.on(db.t_track_tags.f_track==db.t_tracks.id)],
        cache=(cache.ram, 43200),
        cacheable=True
    )
    # return tracks.as_json();
